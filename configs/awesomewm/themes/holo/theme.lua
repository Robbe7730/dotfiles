...
theme.wifi_great                                = theme.icon_dir .. "/wifi-solid.png"
theme.wifi_good                                 = theme.icon_dir .. "/wifi-solid.png"
theme.wifi_mid                                  = theme.icon_dir .. "/wifi-solid.png"
theme.wifi_weak                                 = theme.icon_dir .. "/wifi-solid.png"
theme.wifi_none                                 = theme.icon_dir .. "/wifi-solid.png"
...
theme.musicplr = string.format("spotify")
...
function ()
    os.execute("playerctl -p " .. theme.musicplr .. " prev")
    theme.mpd.update()
end)))
next_icon:buttons(my_table.join(awful.button({}, 1,
function ()
    os.execute("playerctl -p " .. theme.musicplr .. " next")
    theme.mpd.update()
end)))
stop_icon:buttons(my_table.join(awful.button({}, 1,
function ()
    play_pause_icon:set_image(theme.play)
    os.execute("playerctl -p " .. theme.musicplr .. " stop")
    theme.mpd.update()
end)))
play_pause_icon:buttons(my_table.join(awful.button({}, 1,
function ()
    os.execute("playerctl -p " .. theme.musicplr .. " play-pause")
    theme.mpd.update()
end)))
...
-- Get the list of files from a directory. Must be all images or folders and non-empty. 
    ...
    wallpaperList = scanDir("/home/robbe/Pictures/Backgrounds")
...
-- Apply a random wallpaper every changeTime seconds.
    changeTime = 60 * 60
    wallpaperTimer = timer { timeout = 0 }
    wallpaperTimer:connect_signal("timeout", function()
	gears.wallpaper.maximized(wallpaperList[math.random(1, #wallpaperList)], s)
